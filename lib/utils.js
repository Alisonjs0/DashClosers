...
export function normalizeCloserName(name) {
  ...
  const normalized = mapping[firstPart.toLowerCase()];
...
export function isValidReport(row) {
  ...
  const closer = (row["Closer"] || "").toString().toUpperCase().trim();
  const hasReportData = !!(row["ranking"] || row["estrategia"]);
  
  // If it's a regular call row (no report data), it MUST be in the whitelist
  if (!hasReportData) {
    const isWhitelisted = closerWhitelist.some(name => closer === name || closer.includes(name));
