...
import { Search, Bell, RefreshCcw, Database, Menu } from "lucide-react";
...
export default function DashboardContent({ children, loading, lastUpdated, onRefresh, onSheetSettings }) {
  ...
  return (
    <div className="flex-1 min-w-0 max-w-full h-screen bg-background/50 relative overflow-hidden flex flex-col">
      {/* Top Header / Search Bar */}
      <header className="flex-shrink-0 z-40 h-20 bg-background/20 backdrop-blur-3xl border-b border-white/5 px-4 md:px-8 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button 
            className="md:hidden p-2 rounded-xl bg-white/5 border border-white/5 text-slate-400 hover:text-white"
      ...
      {/* Content Area */}
      <div className="flex-1 overflow-y-auto custom-scrollbar p-0 scroll-smooth transition-all duration-500">
