...
export default function Sidebar() {
  ...
  const { user, logout, isMobileMenuOpen, setIsMobileMenuOpen, isSidebarCollapsed, toggleSidebar } = useDashboardContext();
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          "fixed left-0 top-0 h-screen transition-all duration-500 z-50 bg-[#020617] md:bg-card/30 md:backdrop-blur-2xl flex flex-col border-r border-white/5",
      ...
      >
        ...
        <div className="p-6 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.5)] animate-float flex-shrink-0">
              <Zap size={18} className="text-white fill-current" />
            </div>
            {!isSidebarCollapsed && (
              <span className="font-bold text-lg tracking-tight impact-title animate-in fade-in slide-in-from-left-2 duration-300">Dash Closers</span>
            )}
          </div>
      ...
      <nav className="flex-1 mt-6 px-3 space-y-2 overflow-y-auto custom-scrollbar">
        {filteredMenuItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon size={20} className={clsx("flex-shrink-0", isActive ? "text-primary" : "group-hover:text-slate-200")} />
              {!isSidebarCollapsed && (
      ...
      <div className="p-4 border-t border-white/5">
        {user && (
            <div className={clsx(
              "px-4 py-3 bg-white/5 rounded-2xl flex items-center",
              isSidebarCollapsed ? "justify-center" : "gap-3"
            )}>
                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-primary to-cyan-400 flex items-center justify-center text-[10px] font-bold text-white uppercase flex-shrink-0">
                    {user.avatar}
                </div>
                {!isSidebarCollapsed && (
